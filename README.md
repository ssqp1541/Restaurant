...
##### 1.1 코드스멜 분석
- [ ] **전역 변수 제거**
  - [ ] `_restaurants_cache` 전역 변수를 클래스 또는 싱글톤 패턴으로 리팩토링
  - [ ] `_metrics` 전역 변수를 Metrics 클래스로 캡슐화
  - [ ] 전역 상태 의존성 제거

- [ ] **중복 코드 제거**
  - [ ] `load_restaurants_data`와 `save_restaurants_data`의 경로 검증 로직 통합
  - [ ] 검증 함수들의 중복 로직 추출 (`_validate_blog_links`, `_validate_reviews` 등)
  - [ ] 에러 응답 생성 로직 통합

- [ ] **긴 함수 분리**
  - [ ] `health_check` 함수 분리 (파일 시스템 체크, 데이터 로드 체크, 메트릭 추가)
  - [ ] `validate_restaurant_data_with_error` 함수 분리 (각 필드별 검증)
  - [ ] `api_restaurants` 함수의 예외 처리 로직 분리

- [ ] **매직 넘버/문자열 제거**
  - [ ] 하드코딩된 숫자 상수화 (예: `100` → `MAX_RESPONSE_TIMES`)
  - [ ] 하드코딩된 문자열 상수화 (예: `'data/restaurants.json'` → `DEFAULT_DATA_PATH`)
  - [ ] 에러 메시지 상수화
...
- [ ] **pylint/flake8 코드 품질 검사**
  - [ ] 코드 스타일 검사
  - [ ] 복잡도 분석
  - [ ] 코드 품질 점수 향상 (목표: 8.0/10 이상)

- [ ] **순환 복잡도 분석**
  - [ ] 복잡도가 높은 함수 식별 (목표: 10 이하)
  - [ ] 복잡한 조건문 단순화
  - [ ] 중첩 루프 최소화
