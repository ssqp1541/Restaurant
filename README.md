...
- [x] **F6. 통합 테스트 강화** (예상 작업 시간: 5-6시간) ✅ 완료
  - [x] F6.1: 실제 데이터 파일을 사용한 통합 테스트 ✅ 완료
    - [x] 실제 restaurants.json 파일 사용
    - [x] 다양한 데이터 시나리오 테스트 (구조 확인, 검증, 저장/로드 사이클)
  - [x] F6.2: Flask 앱과 데이터 로더 연동 테스트 강화 ✅ 완료
    - [x] 데이터 로드 실패 시나리오
    - [x] 데이터 검증 실패 시나리오

- [x] **F7. API 엔드포인트 개선** (예상 작업 시간: 3-4시간) ✅ 완료
  - [x] F7.1: API 에러 응답 표준화 ✅ 완료
    - [x] 일관된 에러 응답 형식 (성공: `{success: true, data: [...], count: N}`, 에러: `{success: false, error: {...}}`)
    - [x] HTTP 상태 코드 적절한 사용 (200, 400, 500)
    - [x] 에러 코드 체계 구축 (`ERR_400`, `ERR_500`, `ERR_DATA_LOAD_FAILED`)
  - [x] F7.2: API 문서화 ✅ 완료
